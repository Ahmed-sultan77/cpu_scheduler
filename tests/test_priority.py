...
def run_sjf(processes_list):
    ...
    while completed < n:
        # البحث عن العمليات اللي وصلت حالياً ولم تنتهِ
        available_indices = []
        for i in range(n):
        ...
        if len(available_indices) > 0:
            # Loop يدوي لإيجاد العملية ذات أقل Burst Time (SJF)
...
def run_priority(processes_list):
    ...
    while completed < n:
        ...
        if len(available_indices) > 0:
            # Loop يدوي لإيجاد العملية ذات الأولوية الأعلى (أصغر رقم Priority)
            best_idx = available_indices[0]
            for idx in available_indices:
                if procs[idx].priority < procs[best_idx].priority:
                    best_idx = idx
                # في حالة تساوى الأولوية، نأخذ التي وصلت أولاً (FCFS as Tie-breaker)
...
# البيانات المطلوبة للاختبار (Test Scenarios)
# سيناريو يوضح الفرق: P1 طويل ومهم، P2 قصير وغير مهم
test_data = [
